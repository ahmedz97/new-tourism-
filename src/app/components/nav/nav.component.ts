...
export class NavComponent implements OnInit {
  ...
  onLangChange(event: Event): void {
    ...
    if (isPlatformBrowser(this.platformId)) {
      ...
      // Update both select values (desktop and mobile)
      const selectDesktop = document.getElementById(
        'language'
      ) as HTMLSelectElement;
      const selectMobile = document.getElementById(
        'language-mobile'
      ) as HTMLSelectElement;
      if (selectDesktop) selectDesktop.value = langCode;
      if (selectMobile) selectMobile.value = langCode;
    }
  }
  ...
  toggleMobileDropdown(dropdownId: string, event?: Event) {
    if (event) {
